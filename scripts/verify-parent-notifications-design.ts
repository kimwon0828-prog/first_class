...
assert(read("app/page.tsx").includes("hasUnreadNotifications={hasUnreadNotifications}"))
assert(indicator.includes("hasUnreadNotifications = false"))
assert(row.includes("item.isUnread") && row.includes("읽지 않은 알림"))
...
console.log("PASS: Seoul today/year/weekday/time, standalone navigation, pending report indicator, no card unread, row and real retry")
